...
const CardSection = ({ className,productId,productId2, businessName, ...rest }) => {
    const handleClick = () => {
        gtag.event({ action: '$250_card_for_$125', category: gtag.EventCategories.buynow, label: window.location.pathname, value: window.location.pathname });
      };
    const handleClick2 = () => {
        gtag.event({ action: '$100_card_for_$50', category: gtag.EventCategories.buynow, label: window.location.pathname, value: window.location.pathname });
    };   
    return (
